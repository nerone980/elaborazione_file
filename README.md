...
4. **Calcolo**: per ogni riga del file 1, somma quanto trovato nei file da sottrarre sulla stessa chiave e calcola `netto = disponibili - sottratti`. I valori negativi si possono lasciare com'è o azzerare. Le chiavi presenti solo nei file da sottrarre (senza corrispondenza nel file 1) finiscono tra le "chiavi orfane". Un valore di chiave che è un errore Excel (`#N/D`, `#N/A`, `#REF!`, `#VALORE!`/`#VALUE!`, `#DIV/0!`, `#NOME?`/`#NAME?`, `#NULL!`, `#NUM!`) viene trattato come vuoto: la riga non si abbina a nulla, invece di essere unita per errore ad altre righe con lo stesso errore.
